...
1. **Instalar el mismo Syncrash.** Con Imperivm cerrado, cada jugador abre `Syncrash.exe` y pulsa **Aplicar parche**. Si no localiza Steam, selecciona su `gbr.exe`. El mismo mensaje de éxito sirve para instalación y reinstalación. Si se rechaza un archivo, conservar el mensaje y no forzar el parche.
